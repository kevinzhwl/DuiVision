...
BOOL CDuiPanel::LoadXmlFile(CString strFileName)
{
	...
	{
		DuiSystem::LogEvent(DUIV_LOG_LEVELERROR, _T("CDuiPanel::LoadXmlFile %s failed"), strFileName);
		return FALSE;
	}
	...
	if(pDivElem == NULL)
	{
		DuiSystem::LogEvent(DUIV_LOG_LEVELERROR, _T("CDuiPanel::LoadXmlFile %s failed, not found div node"), strFileName);
		return FALSE;
	}
	...
	if(!Load(pDivElem))
	{
		DuiSystem::LogEvent(DUIV_LOG_LEVELERROR, _T("CDuiPanel::LoadXmlFile %s failed, load div node fail"), strFileName);
		return FALSE;
	}
