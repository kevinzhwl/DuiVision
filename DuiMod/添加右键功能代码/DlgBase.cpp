...
void CDlgBase::OnLButtonUp(UINT nFlags, CPoint point)
{
	...
	if (m_pControl)
	{
		if(m_pControl->GetVisible() && m_pControl->GetRresponse())
		{
			...
			//AfxMessageBox(_T("CDlgBase::OnLButtonUp"));
...
/*	CControlBase * pControlBase = GetFocusControl();
	if (pControlBase && (pControlBase->GetNativeHWnd() != NULL))
	{
		::SendMessage(pControlBase->GetNativeHWnd(), MSG_RBUTTON_DOWN, pMsg->wParam, pMsg->lParam);
	}
	*/
	//AfxMessageBox(_T("CDlgBase::OnRButtonUp"));
	if (m_bIsSetCapture)
	{
		ReleaseCapture();
		m_bIsSetCapture = false;
...
BOOL CDlgBase::PreTranslateMessage(MSG* pMsg)
{
	...
	{
		//AfxMessageBox(_T("死胖子，日你妹"));
		m_wndToolTip.RelayEvent(pMsg);
	} else if (pMsg->message == WM_DROPFILES) {
		//AfxMessageBox(_T("死胖子，日你大妹妹"));
	} else if (/*pMsg->message == WM_RBUTTONDOWN || */pMsg->message == WM_RBUTTONUP) {
		//AfxMessageBox(_T("死胖子，日你二妹妹"));
		//DuiSystem::LogEvent(LOG_LEVEL_DEBUG, L"WM_RBUTTONUP:%d,%d", pMsg->wParam, pMsg->lParam);
		//CString str;
		//str.Format(_T("WM_RBUTTONUP:%d,%d"), pMsg->wParam, pMsg->lParam);
		//AfxMessageBox(str);
		//CControlBase* pFocusControl = GetFocusControl();
		//if(pFocusControl && (pFocusControl->GetNativeHWnd() != NULL))
		//{
		//	::SendMessage(pFocusControl->GetNativeHWnd(), MSG_RBUTTON_UP, pMsg->wParam, pMsg->lParam);
		//}
...
{
	//AfxMessageBox(_T("CDlgBase::OnBaseMessage"));
